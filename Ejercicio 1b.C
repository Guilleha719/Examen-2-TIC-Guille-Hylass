#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define NPERSONAS 3

int main(void) {
    char *lista[NPERSONAS];
    char *pintercambio;
    char nombre_fecha[20]; // Suponemos que el formato es nombre_fecha
    char *nombre;
    char *fecha;

    for (int i = 0; i < NPERSONAS; i++) {
        printf("Introduzca el nombre y la fecha de nacimiento de la persona %d (en formato 'nombre_fecha'): ", i + 1);
        scanf("%s", nombre_fecha);

        // Separar el nombre y la fecha usando strtok
        nombre = strtok(nombre_fecha, "_");
        fecha = strtok(NULL, "_");

        // Asignar memoria para el nombre y copiarlo
        lista[i] = (char *)malloc(strlen(nombre) + 1);
        strcpy(lista[i], nombre);

        // Concatenar la fecha al final del nombre
        lista[i] = (char *)realloc(lista[i], strlen(lista[i]) + strlen(fecha) + 2); // +2 para el ' ' y el carácter nulo
        strcat(lista[i], " ");
        strcat(lista[i], fecha);
    }

    // Imprimir la lista con el formato solicitado
    printf("\nLista de nombres y fechas:\n");
    for (int i = 0; i < NPERSONAS; i++) {
        printf("%s\n", lista[i]);
    }

    // Liberar memoria asignada
    for (int i = 0; i < NPERSONAS; i++) {
        free(lista[i]);
    }

    return 0;
}
